...
public final class QueryUtils {
    ...
    public static ArrayList<Data> extractEarthquakes(String json_response) {
        ...
        try {
            ...
            JSONArray features_array = root.getJSONArray("features");
            for(int i = 0; i < features_array.length(); i++){
                ...
                /*
                 * THIS WILL GET THE JSON OBJECT AT INDEX
                 * 'i'
                 */
                JSONObject nth_object = features_array.getJSONObject(i);

                // EXTRACTING DATA FROM nth_object json object:
                JSONObject properties_object = nth_object.getJSONObject("properties");

                //EXTRACTING MAGNITUDE, LOCATION AND TIME FROM properties_object
                mag += properties_object.getDouble("mag");
                location += properties_object.getString("place");
                time += properties_object.getLong("time");
                url += properties_object.getString("url");

                earthquakes.add(new Data(mag, location, time, url));
    ...
    public static ArrayList<Data> fetchEarthquakes (String usgs_response) throws JSONException {

        URL url = createURL(usgs_response);
